...
async def forja_elmo_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    texto = ler_texto("../texts/midtheim/ferreiro/forja/forja_elmo.txt")
    teclado = InlineKeyboardMarkup([
        [InlineKeyboardButton("Criar", callback_data="criar_elmo")],
...
async def criar_elmo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.edit_message_reply_markup(reply_markup=None)
    ...
    elmos = ref.child("Equipamentos/Elmos").get()
    teclado = InlineKeyboardMarkup([
    ...
    if sucesso:
        ...
        equipamento = f"[20/20] [{emoji}] 🪖 Elmo de Ferro [1] [{buffs}]"

        for i in range(1, 11):
            if elmos.get(f"Item{i}") == "":
                ref.child(f"Equipamentos/Elmos/Item{i}").set(equipamento)
                break
