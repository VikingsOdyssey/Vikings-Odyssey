...
from utils.atributos import calcular_atributos, calcular_dano_com_reducao
import os
import random
import time
...
async def iniciar_combate_amistoso(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for j in jogadores:
        ...
        equipamentos = [eq.arma, eq.elmo, eq.armadura, eq.calca, eq.bota]
        if eq.amuleto:
            equipamentos.append(eq.amuleto)
...
async def resolver_combate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    eu_eq = session.query(Equipado).filter_by(chat_id=chat_id).first()
    op_eq = session.query(Equipado).filter_by(chat_id=oponente.chat_id).first()

    meus_attr, minha_vida, minha_agilidade, meu_critico, meu_dano = calcular_atributos(jogador, eu_eq)
    ...
    while vida1 > 0 and vida2 > 0 and rodada <= 10:
        ...
        for atacante, dano_base, chance_crit, defensor_attrs, alvo in ordem:
            ...
            dano_final = calcular_dano_com_reducao(dano_base, critado, atributo_ataque, defensor_attrs)
    ...
    caminho = os.path.join("bot", "textos", "midtheim", "arena", "combate_amistoso.txt")
